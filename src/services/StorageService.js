...
export class StorageService {
  ...
  constructor(storageName) {
    //스토리지 이름이 없을 때
    if (!storageName) {
      throw new Error("스토리지 이름을 입력하세요.");
    }
  ...
  //스토리지 데이터 저장
  #saveStorageData(obj) {
    const json = JSON.stringify(obj); //JSON문자열로 반환
  ...
  //신규 항목 삽입
  addItem(item) {
    const sotrageData = this.#getStorageData();

    //현재 nextId를 항목의 id에 적용하고 1만큼 증가
    item.id = sotrageData.nextId++;

    sotrageData.items.push(item);
    this.#saveStorageData(sotrageData);
  }
  ...
  setItem(item) {
    ...
    for (let i = 0; i < sotrageData.items.length; i += 1) {
      if (sotrageData.items[i].id === item.id) {
        sotrageData.items[i] = item;
        break;
      }
    }
